    ...
    for size in sizes:
        ...
        for strategy in strategies:
            ...
            try:
                ...
                # Note: exec() is safe here - code is generated by our own Strategy classes
                # with controlled inputs, not from external/user sources
                exec_globals = context.variables.copy()
                exec_globals[input_id] = test_data
                exec_locals = {}
                
                # Compile the generated code into a callable for better performance
                # and to reduce the overhead of repeated exec() calls
                code_obj = compile(code, '<strategy_generated>', 'exec')
